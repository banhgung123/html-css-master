...
- width: fit-content: có độ rộng bằng với nội dung nó chứa
...
- _list-style_: thuộc tính này dùng cho thẻ ul(`disc`) và ol(`decimal`)
...
- _input_: có thuộc tính (attribute)
- `type` có nhiều loại tuỳ thuộc vào mục đích chúng ta sử dụng: text, mail, number, phone, time, date, file, password, checkbox, radio...
...
- `readonly` chỉ đọc, không sửa được tuy nhiên khi submit form vẫn lấy được dữ liệu
...
- Trạng thái của input và button có `focus` `valid` `invalid` `out-of-range`
- Button có `hover`, `focus` `active` `disabled`
- select: thường sẽ được tuỳ biến lại bằng cách dùng ul li
- textarea: thường dùng cho nội dung nhập nhiều và có xuống hàng, thư viện soạn thảo hay gặp là `ckeditor`
...
- _auto-fit_: nó tương tự auto-fill nhưng nó lấp đầy khoảng trống còn dư
- pseudo: `:hover` selectors:pseduo -> Search google with keywords: All pseduo class css(W3schools, MDN)
...
- _visibility_: về cơ bản nó giống thằng opacity (`hidden`) -> nhưng không hiển thị con trỏ `pointer`, vẫn chiếm diện tích, lưu ý sử dụng ở phần tử cha thay vì pseduo như before hay after
- _cursor_: con trỏ, có nhiều loại, pointer, not-allowed, cross ...
- _content_: sử dụng trong pseduo before và after, nội dung có thể có hoặc không tuỳ vào mục đích
...
- _counter_: thường để đánh số thứ tự, thích hợp khi làm mục lục
